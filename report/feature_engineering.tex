...
\subsection{Information Theoretic Features}
In addition to spectral features, we extracted non-linear features to capture the complexity and connectivity of the EEG signals, as these dynamics are often altered in ADHD pathology.

\begin{enumerate}
    \item \textbf{Shannon Entropy:} 
    Calculated for each channel to quantify the unpredictability or "randomness" of the signal amplitude distribution. Higher entropy typically correlates with increased cortical noise.
    
    \item \textbf{Mutual Information (MI):} 
    Computed between channel pairs (e.g., F3-F4) to measure non-linear functional connectivity. Unlike correlation, MI captures the reduction in uncertainty about one signal given the knowledge of another, serving as a proxy for neural synchronization.
\end{enumerate}
...
represents a distinct subject and columns correspond to the extracted spectral features (Band Powers, TBR) and information-theoretic metrics 
(Entropy, Mutual Information) for each channel.
...
Although Information Theoretic features (Entropy, Mutual Information) were included in the feature set, the spectral features (specifically 
the Theta/Beta Ratio) proved to be the most discriminative for this classifier, occupying the top ranks in Gini Importance.
